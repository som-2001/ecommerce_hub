...
export const ecomSlice = createSlice({
  ...
  reducers: {
    ...
    removeItem: (state, action) => {
      const removedItem = state.body.find(
      ...
      if (removedItem) {
        ...
        state.total = parseFloat(
          (state.total - removedItemTotalPrice).toFixed(2)
        );
      }
      state.body = state.body.filter((item) => item.id !== action.payload.id);
      state.id = state.id.filter((item) => item !== action.payload.id);
...
export const { setBody, increaseCount, decreaseCount, removeItem } =
  ecomSlice.actions;
